let audio_hit = document.getElementById("audio_hit"),
	...
	renderer,
	heightBorderGeometry, widthBorderGeometry, borderMaterial,
	leftBorder, rightBorder, bottomBorder, topBorder,
	mainLight, leftLight, rightLight,
	textMaterial, textGeom, textMesh,
	currentTet,
	nextTet,
	dropSpeed = 1000,
	gameIsOver = false,
	...
	stats = new Stats(),
	upLastPressed = 0, lastDropped = 0, leftLastPressed = 0, rightLastPressed = 0, spaceLastPressed = 0,
...
// Create the border
heightBorderGeometry = new THREE.BoxGeometry(.5, 20, 1),
...
// Add the lights
mainLight = new THREE.PointLight(0xFFFFFF, .8);
mainLight.position.set(4.5, 9.5, 12);
scene.add(mainLight);

leftLight = new THREE.PointLight(0xAAAAFF, .6);
leftLight.position.set(-2, 9.5, 0);
scene.add(leftLight);

rightLight = new THREE.PointLight(0xFFAAAA, .6);
